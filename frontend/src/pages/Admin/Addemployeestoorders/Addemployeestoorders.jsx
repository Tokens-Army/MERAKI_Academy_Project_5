...
function employeeAdded() {
  alert(`${employee.name} employee was added to order number ${order.id}`);
}
const Addemployeestoorders = () => {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/orders/employees/employees")
      .then((results) => {
      ...
      <div className="ordersCard">
        ...
        {data.orders ? (
          data.orders.map((order) => {
            return (
              <div className="ordersInfo" key={order.id}>
                <div>{order.id}</div>
                <div>{order.created_at}</div>
                {order.order_status === "pending" ? (
                  <div className="orderStatusPending">{order.order_status}</div>
                ) : (
                  <div className="orderStatusAccepted">
                    {order.order_status}
                  </div>
                )}
                {order.employee_id ? (
                        ...
                        <div>
                          <div className="employeeCard">
                            <div className="employeeInfo">
                              <span className="disc">Id</span>
                              <span className="disc">Name</span>
                              <span className="disc">Image</span>
                              <span className="disc">Phone number</span>
                              ...
                              data.employees.map((employee) => {
                                return (
                                  ...
                                  >
                                    ...
                                    <div className="results_availability">
                                    ...
                                    <button
                                      ...
                                      onClick={() => {
                                        axios
                                          .put(
                                            `http://localhost:5000/orders/addemployees/${order.id}/${employee.id}`
                                          )
                                          .then((results) => {
                                            dispatch(
                                              addEmployee({
                                            ...
                                            alert(
                                              `${employee.name} employee was added to order number ${order.id}`
                                            );
                                          })
                                          .catch((err) => {
                                            console.log(err);
